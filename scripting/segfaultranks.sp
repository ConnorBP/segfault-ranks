...
bool g_bEnabled = true;  // TODO add a convar to disable this plugin, and verify that the database
                         // is handles properly when the plugin is disabled while already running
...
public
void OnPluginStart() {
    // g_cvarDebugEnabled = CreateConvar(DEBUG_CVAR, "segfaultranks", "is segfaultranks debugging
    // filename?");
    ...
    g_AllowStatsOtherCommandCvar =
        CreateConVar("sm_segfaultranks_allow_stats_other", "1",
                     "Whether players can use the .rws or !rws command on other players");
    // g_RecordStatsCvar = CreateConVar("sm_segfaultranks_record_stats", "1", "Whether rws should be
    // recorded while not in warmup (set to 0 to disable changing players rws stats)");
    // g_cvarAutopurge = CreateConVar("sm_segfaultranks_autopurge", "0", "Auto-Purge inactive
    // players? X = Days  0 = Off", _, true, 0.0);
    g_cvarMinimumPlayers = CreateConVar("sm_segfaultranks_minimumplayers", "2",
                                        "Minimum players to start giving points", _, true, 0.0);
    g_cvarRankMode =
        CreateConVar("sm_segfaultrank_rank_mode", "1",
                     "Rank by what? 1 = by rws 2 = by kdr 3 = by elo", _, true, 1.0, true, 3.0);
    // g_cvarDaysToNotShowOnRank = CreateConVar("sm_segfaultrank_timeout_days", "0", "Days inactive
    // to not be shown on rank? X = days 0 = off", _, true, 0.0);
    g_cvarMinimalRounds =
        CreateConVar("sm_segfaultrank_minimal_rounds", "0",
                     "Minimal rounds played for rank to be displayed", _, true, 0.0);
    // g_RankEachRoundCvar = CreateConVar("sm_segfaultranks_rank_rounds", "1", "Sets if Elo ranks
    // should be updated every round instead of at match end. (Useful for retake ranking)");
    // g_SetEloRanksCvar = CreateConVar("sm_segfaultranks_display_elo_ranks", "2", "Wether or not to
    // display user ranks based on calculated total ELO. (S,G,A,B,etc) 0=Don't Display 1=Calculate
    // elo ranks 2=Use top RWS", _, true, 0.0, true, 2.0); g_ShowRWSOnScoreboardCvar =
    // CreateConVar("sm_segfaultranks_display_rws", "1", "Whether rws stats for current map are to
    // be displayed on the ingame scoreboard in place of points."); g_ShowRankOnScoreboardCvar =
    // CreateConVar("sm_segfaultranks_display_rank", "1", "Whether rws stats for current map are to
    // be displayed on the ingame scoreboard in place of points.");
    g_cvarRankCache =
        CreateConVar("sm_segfaultranks_rank_cache", "0",
                     "Get player rank via cache, auto build cache on every OnMapStart.", _, true,
                     0.0, true, 1.0);
    ...
    g_hOnHelpCommand = CreateGlobalForward("SegfaultRanks_OnHelpCommand", ET_Ignore, Param_Cell,
...
Action Event_RoundEnd(Event event, const char[] name, bool dontBroadcast) {
    if (!ShouldRank()) {
        // reset round points here anyways so that they don't accidentaly affect the first real
        // round
        // TODO
...
public
bool IsOnDb(int client) {
    return userData[client].on_db;
}

// Re-Usable checks for wether or not we should rank players right now
bool ShouldRank() {
    // ranks should be calculated if it is not warmup, and there are at least the min player count
    // (2 by default)
...
Action Command_DumpRWS(int client, int args) {
    for (int i = 1; i <= MaxClients; i++) {
        if (IsPlayer(i) && IsOnDb(i)) {
            ReplyToCommand(client, "%L has RWS=%f, roundsplayed=%d", i, userData[i].RWS,
                           userData[i].ROUNDS_TOTAL);
        }
    }
...
Action Command_RWS(int client, int args) {
    ...
    if (target != -1) {
        if (IsOnDb(target)) {
            SegfaultRanks_Message(client, "%N has a RWS of %.1f with %d rounds played", target,
...
public
Action Command_Leaderboard(int client, int args) {
    return Plugin_Handled;
}
