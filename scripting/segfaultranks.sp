...
Action Event_RoundEnd(Event event, const char[] name, bool dontBroadcast) {
  if (!ShouldRank()) {
    // reset round points here anyways so that they don't accidentaly affect the
    // first real round
    // TODO
...
#define minimumEnemies 1
  ...
  LogDebug("RoundUpdate(%L), alpha=%f, round_rws=%f, new_rws=%f", client, alpha,
           rws, userData[client].RWS);
}
...
bool ShouldRank() {
  // ranks should be calculated if it is not warmup, and there are at least the
  // min player count (2 by default)
...
Action Command_RWS(int client, int args) {
  ...
  if (target != -1) {
    if (IsOnDb(target)) {
      ...
      SegfaultRanks_Message(client, "%N does not currently have stats stored",
                            target);
    }
  } else {
