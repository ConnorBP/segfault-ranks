public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) {
    ...
    CreateNative("SegfaultRanks_MessageToAllExcept", Native_MessageToAllExcept);
...
public int Native_MessageToAllExcept(Handle plugin, int numParams) {
  int client = GetNativeCell(1);
  char prefix[64];
  cvarMessagePrefix.GetString(prefix, sizeof(prefix));
  char buffer[1024];
  ...
  for (int i = 0; i <= MaxClients; i++) {
    if (i == client || (i != 0 && (!IsClientConnected(i) || !IsClientInGame(i))))
      continue;
